...
| `timeout`                | number                       | `5000`                                                                                   | Time (in milliseconds) to display the static part of the notification. The total display time depends on `nvim-notify`'s `stages` setting: <br>- `fade_in_slide_out` (default) and `fade`: Add ~2.4s to the duration. <br>- `slide`: Adds ~1.3s to the duration. <br>- `static`: Matches `timeout` (no extra delay).<br>**Note:** Only applies if `nvim-notify` is installed and enabled. |
